...
function initialize() {
	...
	try {
		...
		// create the Xwing2 named object
		useLibrary( 'res://namedobj.js' );
		const Xwing2 = Eons.namedObjects.Xwing2;

		// register X-wing 2ed as a game supported by Strange Eons
		Game.register('XW2', @xw2_game, #xw2_game, ImageUtils.get( 'plugin.png' ), template);
				
		// now that the game is regsietered, we can add the
		// default settings to use for new cards
		Game.get( 'XW2' ).masterSettings.addSettingsFrom( 'base.settings' );
