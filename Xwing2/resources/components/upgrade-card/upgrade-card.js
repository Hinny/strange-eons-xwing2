...
function create( diy ) {
	...
	diy.name = #xw2-upgrade-name;
	$UpgradeType = #xw2-upgrade-type;
	$UniqueUpgrade = #xw2-upgrade-unique;
	$DoubleIcon = #xw2-upgrade-double;
	$Dual = #xw2-upgrade-dual;
	$SubName = #xw2-upgrade-sub;
	$Restriction = #xw2-upgrade-restriction;
	$PointCost = #xw2-upgrade-cost;

	$EnergyLimit = #xw2-upgrade-energylimit;
	$SecondaryWeapon = #xw2-upgrade-weapon;
	$Style = #xw2-upgrade-style;
	$AttackValue = #xw2-upgrade-attack;
	$Range = #xw2-upgrade-range;
	$Text = #xw2-upgrade-text;

	$DualSubName = #xw2-upgrade-dual-sub;
	$DualEnergyLimit = #xw2-upgrade-dual-energylimit;
	$DualSecondaryWeapon = #xw2-upgrade-dual-weapon;
	$DualAttackValue = #xw2-upgrade-dual-attack;
	$DualRange = #xw2-upgrade-dual-range;
	$DualText = #xw2-upgrade-dual-text;
}
...
function createInterface( diy, editor ) {
	...
	upgradeItems = [];
	upgradeItems[0] = ListItem( 'elite', @xw2-upgrade-elite );
	...
	upgradeItems[12] = ListItem( 'hardpoint', @xw2-upgrade-hardpoint );
	upgradeItems[13] = ListItem( 'team', @xw2-upgrade-team );
	upgradeItems[14] = ListItem( 'cargo', @xw2-upgrade-cargo );
	upgradeItems[15] = ListItem( 'modification', @xw2-upgrade-modification );
	upgradeItems[16] = ListItem( 'title', @xw2-upgrade-title );
	...
	doubleCheckbox = checkBox( @xw2-double );
	bindings.add( 'DoubleIcon', doubleCheckbox, [0,1] );

	dualCheckbox = checkBox( @xw2-dual );
	bindings.add( 'Dual', dualCheckbox, [0,1] );

	styleItems = [];
	styleItems[0] = ListItem( 'regular', @xw2-style-regular );
	styleItems[1] = ListItem( 'full', @xw2-style-full );
	styleBox = comboBox( styleItems );
	bindings.add( 'Style', styleBox, [0,1] );

	restrictionItems = [ #xw2-restriction-limited, #xw2-restriction-rebel, #xw2-restriction-imperial, #xw2-restriction-scum, #xw2-restriction-small, #xw2-restriction-large, #xw2-restriction-huge ];
	...
	commonPanel.place( @xw2-upgradename, '', nameField, 'span, growx, wrap' );
	...
	commonPanel.place( @xw2-style, '', styleBox, 'span, growx, wrap para' );	
	commonPanel.place( separator(), 'span, growx, wrap para' );
	commonPanel.place( @xw2-upgradetype, '', typeBox, 'span, growx, wrap para' );
	commonPanel.place( doubleCheckbox, 'wrap para' );
	commonPanel.place( @xw2-upgraderestriction, '', upgradeRestrictionField, 'span, growx, wrap' );
	commonPanel.place( separator(), 'span, growx, wrap para' );
	commonPanel.place( @xw2-pointcost, 'span 2', pointCostSpinner, 'wrap para');
	commonPanel.place( separator(), 'span, growx, wrap para' );
	...
	weaponCheckbox = checkBox( @xw2-weapon );
	...
	upgradePanel.panelTitle = @xw2-portrait-upgrade;

	frontPanel = new Grid( '', '[min:pref][min:pref,grow][min:pref][min:pref,grow]', '');
	frontPanel.place( @xw2-subname, '', subNameField, 'span, growx, wrap' );
	frontPanel.place( separator(), 'span, growx, wrap para' );
	frontPanel.place( @xw2-energylimit, '', energyLimitBox, 'wmin 70, span 2, wrap para' );
	frontPanel.place( separator(), 'span, growx, wrap para' );
	frontPanel.place( weaponCheckbox, 'wrap para' );
	frontPanel.place( @xw2-attackvalue, '', attackValueBox, 'wmin 70, span 2, wrap' );
	frontPanel.place( @xw2-range, '', rangeBox, 'wmin 70, span 2, wrap para' );
	frontPanel.place( separator(), 'span, growx, wrap para' );
	frontPanel.place( @xw2-upgradetext, 'span, growx, wrap' );
	...
	dualWeaponCheckbox = checkBox( @xw2-weapon );
	...
	dualSymbolsTagTip = tipButton( @xw2-symbol-tooltip );
	dualHeadersTagTip = tipButton( @xw2-header-tooltip );
	dualShipsTagTip = tipButton( @xw2-ship-tooltip );

	dualUpgradePanel = portraitPanel( diy, 1 );
	dualUpgradePanel.panelTitle = @xw2-portrait-upgrade;

	backPanel = new Grid( '', '[min:pref][min:pref,grow][min:pref][min:pref,grow]', '');
	backPanel.place( @xw2-subname, '', dualSubNameField, 'span, growx, wrap' );
	backPanel.place( separator(), 'span, growx, wrap para' );
	backPanel.place( @xw2-energylimit, '', dualEnergyLimitBox, 'wmin 70, span 2, wrap para' );
	backPanel.place( separator(), 'span, growx, wrap para' );
	backPanel.place( dualWeaponCheckbox, 'wrap para' );
	backPanel.place( @xw2-attackvalue, '', dualAttackValueBox, 'wmin 70, span 2, wrap' );
	backPanel.place( @xw2-range, '', dualRangeBox, 'wmin 70, span 2, wrap para' );
	backPanel.place( separator(), 'span, growx, wrap para' );
	backPanel.place( @xw2-upgradetext, 'span, growx, wrap' );
...
function createFrontPainter( diy, sheet ) {
	nameBox = Xwing2.headingBox( sheet, 12 );
	
	upgradeTextBox = Xwing2.abilityBox( sheet, 7 );
	
	upgradeIconBox = Xwing2.abilityBox( sheet, 14 );
}
...
function paintCardFaceComponents( g, diy, sheet, side) {
	...
	if( secondaryWeapon ) { // secondary weapon
		if ( energyLimit == '-' ) {
			...
			sheet.drawOutlinedTitle( g, attackValue, R( 'upper-attribute' ), Xwing2.numberFont, 14, 1, Xwing2.getColor('attack'), Color.BLACK, sheet.ALIGN_CENTER, true);
			sheet.drawOutlinedTitle( g, range, R('upper-range'), Xwing2.numberFont, 8, 1, Color.WHITE, Color.BLACK, sheet.ALIGN_CENTER, true);
			nameBox.draw( g, R('short-name') );
		} else {
			sheet.paintImage( g, 'upgrade-energy-attack-template', 0, 0 );
			sheet.drawOutlinedTitle( g, energyLimit, R( 'upper-attribute' ), Xwing2.numberFont, 14, 1, Xwing2.getColor('energy'), Color.BLACK, sheet.ALIGN_CENTER, true);
			sheet.drawOutlinedTitle( g, attackValue, R( 'lower-attribute' ), Xwing2.numberFont, 14, 1, Xwing2.getColor('attack'), Color.BLACK, sheet.ALIGN_CENTER, true);
			sheet.drawOutlinedTitle( g, range, R('lower-range'), Xwing2.numberFont, 8, 1, Color.WHITE, Color.BLACK, sheet.ALIGN_CENTER, true);
			nameBox.draw( g, R('short-name') );
		}
	...
	if( $UpgradeType == 'modification' ) {
		if( restriction ) {
			restriction = restriction + ' ' + #xw2-restriction-modification;
		} else {
			restriction = #xw2-restriction-modification;
		}
	} else if( $UpgradeType == 'title' ) {
		if( restriction ) {
			restriction = restriction + ' ' + #xw2-restriction-title;
		} else {
			restriction = #xw2-restriction-title;
		}
	}
	if( $$Dual.yesNo ) {
		if( restriction ) {
			restriction = restriction + ' ' + #xw2-restriction-dual;
		} else {
			restriction = #xw2-restriction-dual;
		}
	}
	
	if( restriction ) {
		restriction = Xwing2.smallCaps( restriction );
	...
	if (style == 'full' ){ // full art: different region
		sheet.drawOutlinedTitle( g, $PointCost, R('cost-alt'), Xwing2.numberFont, 8, 0.5, Color.BLACK, Color.WHITE, sheet.ALIGN_CENTER, true);
	} else { // regular art
		sheet.drawOutlinedTitle( g, $PointCost, R('cost'), Xwing2.numberFont, 8, 0.5, Color.BLACK, Color.WHITE, sheet.ALIGN_CENTER, true);
	}
