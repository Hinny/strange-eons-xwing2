...
function create( diy ) {
	...
	diy.name = #xw2-condition-name;
	$UniqueCondition = #xw2-condition-unique;
	$ConditionText = #xw2-condition-text;
}
...
function createInterface( diy, editor ) {
	...
	var uniqueCheckbox = checkBox( @xw2-unique );
    ...
    var symbolsTagTip = tipButton( @xw2-symbol-tooltip );
    var headersTagTip = tipButton( @xw2-header-tooltip );
    var shipsTagTip = tipButton( @xw2-ship-tooltip );
	
	var conditionPortrait = portraitPanel( diy, 0, @xw2-condition-portrait-shared );
	var portraitApply = button(@xw2-condition-portrait-apply, null, applyPortraits);

	var panel = new Grid( '', '[min:pref][min:pref,grow][min:pref][min:pref,grow]', '');
	panel.place( @xw2-condition-name, '', nameField, 'span, growx, wrap' );
	panel.place( uniqueCheckbox, 'wrap para' );
	panel.place( separator(), 'span, growx, wrap para' );
	panel.place( @xw2-condition-common, 'span, growx, wrap' );
	...
	panel.addToEditor( editor, @xw2-info );

	bindings.bind();

	var frontPortrait = portraitPanel(diy, 1, @xw2-condition-portrait-front);
	var backPortrait = portraitPanel(diy, 2, @xw2-condition-portrait-back);
	var markerPortrait = portraitPanel(diy, 3, @xw2-condition-portrait-marker);
	...
	imagesPanel.addToEditor( editor, @xw2-condition-images );
}
...
function createFrontPainter( diy, sheet ) {
	...
	applyPortraits();
	nameBox = Xwing2.headingBox( sheet, 11.5 );
	nameBox.defaultStyle.add( POSTURE, POSTURE_OBLIQUE );
	textBox = Xwing2.abilityBox( sheet, 7 );
